...
class SP_OP_CreateGroup(bpy.types.Operator):
    bl_idname = "sp.create_group"
    ...
    def poll(cls, context):
        return context.space_data.type == "NODE_EDITOR" and context.space_data.tree_type == 'SimpleNodeTree'

    def execute(self, context):
        tree = context.space_data.edit_tree
        orig_node = tree.nodes[self.node]

        new_node_tree = bpy.data.node_groups.new("Simple Group", "SimpleNodeTreeGroup")
...
class SimpleNodeGroup(bpy.types.NodeCustomGroup, SimpleNodeBase):
    bl_idname = 'SimpleNodeGroup'
    ...
    def nested_tree_filter(self, context):
        ...
        if context.bl_idname != 'SimpleNodeGroup':  # It should be our dedicated to this class
            return False
        else:
    ...
    def draw_buttons(self, context, layout):
        ...
        if self.node_tree:
            ...
            layout.operator('sp.create_group', text='Creat Group', icon='ADD').node = self.name
...
def register():
    bpy.utils.register_class(SP_OP_CreateGroup)
    bpy.utils.register_class(SimpleNodeGroup)


def unregister():
    bpy.utils.unregister_class(SP_OP_CreateGroup)
    bpy.utils.unregister_class(SimpleNodeGroup)
